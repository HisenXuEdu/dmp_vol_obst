...
# Obstacle
obst_center = np.array([0.35, 0.3])
obst_axis = np.array([0.01, 0.01])
density_mesh = 10
...
def ext_f_psa(x, v):
    ...
    return f/len(list_obst_steering_point)
...
plt.ylabel(r'$x_2$', fontsize=16)
plt.xlim(-1.1, 0.25)
plt.title('Dynamic volume')
plt.legend(loc='best')

# Trajectory error
plt.figure()
plt.plot(t_des, err_static_point, linestyle=(0, (1, 1)), color = 'red', label = 'static_point')
plt.plot(t_des, err_dynamic_point, linestyle=(0, (3, 2)), color = 'purple', label = 'dynamic_point')
plt.plot(t_des, err_steering_point, linestyle=(0, (3, 2, 1, 2)), color = 'green', label = 'steering_point')
plt.plot(t_des, err_static_volume, linestyle=(0, (3, 2, 1, 2, 1, 2)), color = 'blue', label = 'static_volume')
plt.plot(t_des, err_dynamic_volume, '-', color = 'fuchsia', label = 'dynamic_volume')
...
plt.plot(t_sp[t_range], np.linalg.norm(ddx_track_point_static, axis=1)[t_range], linestyle=(0, (1, 1)), color = 'red', label = 'point_static')

t_sp = np.linspace(0.0, 1.0, np.shape(ddx_track_point_dynamic)[0])
t_range = np.logical_and(t_sp < 0.9, t_sp > 0.4)
plt.plot(t_sp[t_range], np.linalg.norm(ddx_track_point_dynamic, axis=1)[t_range], linestyle=(0, (3, 2)), color = 'purple', label = 'point_dynamic')

t_sp = np.linspace(0.0, 1.0, np.shape(ddx_track_point_steering)[0])
t_range = np.logical_and(t_sp < 0.9, t_sp > 0.4)
plt.plot(t_sp[t_range], np.linalg.norm(ddx_track_point_steering, axis=1)[t_range], linestyle=(0, (3, 2, 1, 2)), color = 'green', label = 'point_steering')

t_sp = np.linspace(0.0, 1.0, np.shape(ddx_track_static_volume)[0])
t_range = np.logical_and(t_sp < 0.9, t_sp > 0.4)
plt.plot(t_sp[t_range], np.linalg.norm(ddx_track_static_volume, axis=1)[t_range], linestyle=(0, (3, 2, 1, 2, 1, 2)), color = 'blue', label = 'static_volume')

t_sp = np.linspace(0.0, 1.0, np.shape(ddx_track_dynamic_volume)[0])
t_range = np.logical_and(t_sp < 0.9, t_sp > 0.4)
plt.plot(t_sp[t_range], np.linalg.norm(ddx_track_dynamic_volume, axis=1)[t_range], '-', color = 'fuchsia', label = 'dynamic_volume')
