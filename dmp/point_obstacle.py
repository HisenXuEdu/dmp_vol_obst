...
class Obstacle_Steering():
    ...
    def gen_external_force(self, x, dx, goal):
        ...
        if np.linalg.norm(dx) > 1e-5:
            ...
            dis = np.linalg.norm(pos)+1e-1
            if dis >= 1:
                return pval

            dphi = self.gamma * phi * np.exp(-self.beta * phi)/abs(np.log(np.sqrt(1-dis**10)))
            r = np.cross(pos, dx)
            ...
            # Check if the perturbation is too large
            # if np.linalg.norm(pval) > np.linalg.norm(dx):
            #     pval = pval / np.linalg.norm(pval) * np.linalg.norm(dx)
